...
/* 
Julia and Kate are still studying dogs. This time they are want to figure out if 
the dogs in their are eating too much or too little food.
...
food portion (recFood) and add it to the object as a new property. Do NOT create a new array, 
simply loop over the array (We never did this before, so think about how you can do this without 
creating a new array).
2. Find Sarah's dog and log to the console whether it's eating too much or too little. HINT: 
Some dogs have multiple users, so you first need to find Sarah in the owners array, and so this
 one is a bit tricky (on purpose) 🤓
3. Create an array containing all owners of dogs who eat too much (ownersTooMuch) and an array 
with all owners of dogs who eat too little (ownersTooLittle).
...
used in 6.)
8. Group the dogs into the following 3 groups: 'exact', 'too-much' and 'too-little', based on whether they
 are eating too much, too little or the exact amount of food, based on the recommended food portion.
...
const { ownersTooMuch, ownersTooLittle } = dogs.reduce(
  (acc, dog) => {
    ...
    return acc; // Missing return statement added
  },
...
const okayAmountOfFood = dogs.filter(checkEatingOk);
...
const groupedByNumberOfOwners = Object.groupBy(dogs, dog => dog.owners.length);
console.log(groupedByNumberOfOwners);

// 10

const sortedRecFoodArr = dogs.toSorted((a, b) => a.recFood - b.recFood);
console.log(sortedRecFoodArr);
